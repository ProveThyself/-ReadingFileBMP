...
// Функция для чтения 2-байтового значения (little-endian) из файла
unsigned short Read2(FILE* f) {
    unsigned char buf[2];
    fread(buf, 1, 2, f);  // Читаем 2 байта из файла
    return buf[0] | (buf[1] << 8);  // Объединяем байты в 16-битное значение
}

// Функция для чтения 4-байтового значения (little-endian) из файла
unsigned int Read4(FILE* f) {
    unsigned char buf[4];
    fread(buf, 1, 4, f);  // Читаем 4 байта из файла
    // Объединяем байты в 32-битное значение
    return buf[0] | (buf[1] << 8) | (buf[2] << 16) | (buf[3] << 24);
}

// Структура для заголовка BMP файла
struct bmp_file_header {
    char type[2];        // Сигнатура "BM"
...
// Структура для информационного заголовка BMP (DIB header)
struct bmp_info_header {
    ...
    unsigned short planes;    // Число плоскостей (всегда 1)
    unsigned short bitCount;  // Бит на пиксель (1, 4, 8, 16, 24, 32)
    unsigned int compression; // Тип сжатия (0 для несжатых)
    unsigned int sizeImage;   // Размер пиксельных данных
    int xpixPerMeter;         // Горизонтальное разрешение (пикс/м)
    int ypixPerMeter;         // Вертикальное разрешение (пикс/м)
...
static unsigned char* read_bmp(const char* fname, int* _w, int* _h) {
    ...
    fread(fh.type, 1, 2, f);  // Читаем сигнатуру (2 байта)
    ...
    ih.size = Read4(f);        // Размер DIB заголовка
    ...
    // Выделяем память под сырые данные и декодированное изображение
    ...
    for (int j = 0; j < h; ++j) {
        for (int i = 0; i < w; ++i) {
            ...
            int bitIndex = 7 - (i % 8);           // Индекс бита в байте (0-7)
